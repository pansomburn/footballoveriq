...
function TeamCrest({ name, size = 28 }: { name: string; size?: number }) {
  ...
  const colors: Record<string, string> = {
    ...
    RMA: '#FEBE10', BAR: '#A50044', ATL: '#CB3524', CHE: '#034694',
    LIV: '#C8102E', PSG: '#004170', MCI: '#6CABDD', LEV: '#E32221',
    JUV: '#000000', INT: '#010E80', MIL: '#FB090B',
  }
...
export function PreMatchCard({ match: m, index, onBookmark, onOpenDetail }: Props) {
    ...
    return (
      ...
      >
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 7 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              <span style={{ fontSize: 14, fontWeight: 600, color: 'var(--text-1)' }}>{m.homeTeam}</span>
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <TeamCrest name={m.awayTeam} size={24} />
              <span style={{ fontSize: 14, fontWeight: 600, color: 'var(--text-1)' }}>{m.awayTeam}</span>
            </div>
          </div>
        ...
        <div style={{
          ...
          {[
            { val: m.factors.oddsOver25,    lbl: 'O2.5' },
            { val: m.factors.oddsBtts,      lbl: 'BTTS' },
            { val: `${m.factors.h2hOver}%`, lbl: 'H2H' },
            { val: m.factors.avgGoals,      lbl: 'AVG G' },
          ].map(f => (
            <div key={f.lbl} style={{
              background: 'var(--bg-elev-2)', borderRadius: 8,
              padding: '7px 4px', textAlign: 'center',
            }}>
              <div style={{ fontFamily: 'var(--font-mono)', fontSize: 13, fontWeight: 600, color: 'var(--text-1)' }}>{f.val}</div>
              <div style={{ fontSize: 9, color: 'var(--text-3)', marginTop: 2, textTransform: 'uppercase', letterSpacing: '0.04em' }}>{f.lbl}</div>
            </div>
          ))}
        </div>
        ...
        {m.injuredKey && (
          <div style={{ marginTop: 10 }}>
            <span style={{
              ...
              background: 'var(--red-soft)', border: '1px solid rgba(239,72,86,.25)',
              color: 'var(--red)', fontSize: 11, padding: '3px 9px', borderRadius: 999,
            }}>🚑 {m.injuredKey} เจ็บ/ไม่แน่ใจ</span>
          </div>
        )}
  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12 }}>
        ...
        <div style={{ flex: 1 }} />
        <span style={{ fontFamily: 'var(--font-mono)', fontSize: 12, color: 'var(--text-3)' }}>⏰ {fmt(m.kickoffTime)}</span>
        <BookmarkBtn bookmarked={m.bookmarked} onClick={e => { e.stopPropagation(); onBookmark(m.id) }} />
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr auto 1fr', alignItems: 'center', gap: 16, marginBottom: 14 }}>
        ...
        <div style={{
          ...
          <span style={{ fontFamily: 'var(--font-mono)', fontSize: 17, fontWeight: 600, color: 'var(--text-1)', lineHeight: 1 }}>{fmt(m.kickoffTime)}</span>
          <span style={{ fontSize: 10, color: 'var(--text-3)', marginTop: 3, textTransform: 'uppercase', letterSpacing: '0.05em' }}>เวลาแข่ง</span>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(6, 1fr)', gap: 6, paddingTop: 12, borderTop: '1px solid var(--hairline)', marginBottom: 12 }}>
        {[
          { val: m.factors.oddsOver25,    lbl: 'O2.5' },
          { val: m.factors.oddsOver35,    lbl: 'O3.5' },
          { val: m.factors.oddsBtts,      lbl: 'BTTS' },
          { val: `${m.factors.h2hOver}%`, lbl: 'H2H' },
          { val: m.factors.avgGoals,      lbl: 'AVG G' },
          ...
          <div key={f.lbl} style={{ textAlign: 'center' }}>
            <div style={{ fontFamily: 'var(--font-mono)', fontSize: 14, fontWeight: 600, color: f.highlight ? accentColor : 'var(--text-1)' }}>{f.val}</div>
            <div style={{ fontSize: 10, marginTop: 2, textTransform: 'uppercase', letterSpacing: '0.05em', color: 'var(--text-3)' }}>{f.lbl}</div>
          </div>
        ))}
      </div>
